...
genApiTS() {
    node bin/gen_socket_apisTS.js >> ${API_JS}
}
...
generateFromRemoteSockets() {
    ...
    if [ "${SOCKETS}" != "" ]; then
        for socket in "${SOCKETS}"; do
            getSocketSchemaFromURL ${INSTANCE_NAME} ${socket} | genApiTS
        done
    fi
}
...
generateFromLocalSockets() {
    for socket in $(ls syncano 2>/dev/null || true); do
        if [ -f "syncano/${socket}/socket.yml" ]; then
            getSocketSchemaFromYAML ${socket} | genApiTS
        fi
    done
}
